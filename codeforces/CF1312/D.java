package com.example.demo.codeforces.CF1312;
...
public class D {
    ...
    static long[] fac = fac(300005, mod);
    static long[] facR = facR(300005, mod);
    static long[] pw = pow(2, 300005, mod);
    public static void solve() {
        ...
        for (int i = n-1; i <= m; i++) {
//            for (int j = 1; j <n-1 ; j++) {
//                //System.out.println((i-2) +" "+ (j-1) + " " + (i-2-(j-1)) + " " + (n-(j+2)));
//
////                ans = (ans + (i-1)*(comb(i-2,j-1))%mod * (comb(i-2-(j-1), n-(j+2))))%mod;
//
//            }
            ans = (ans + (i-1)*(comb(i-2, n-3))%mod*(pw[n-3]))%mod;
        }
        out.println(ans);
    }
